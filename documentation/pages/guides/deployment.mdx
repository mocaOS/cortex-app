...
One command brings up the whole stack — Cortex, Cortex Chat, Neo4j, a nightly backup sidecar, and Caddy for automatic HTTPS if you deploy on a domain — from prebuilt, version-pinned images. Nothing to clone, nothing to build, no Compose file to edit.
...
2. **Live credential validation** — a real chat completion and a real embedding call. Nothing is written to disk and no image is pulled until both succeed, so a wrong key costs you seconds instead of a 1.7 GB pull. The embedding dimension is measured here rather than assumed, because it gets baked into the Neo4j vector index on first use.
3. **Interactive configuration** — localhost or a public domain, your provider and models (embeddings may come from a different provider), your admin identity, and secrets, which are generated for you unless you'd rather set them.
...
Your `.env`, your `Caddyfile` and any `docker-compose.override.yml` survive that copy: the release ships `.env.example` and `Caddyfile.template` and carries no override file. If you never hand-edited `Caddyfile`, re-copy it from the refreshed template so template changes land too.
