...
description: Complete Python code examples for the Cortex API
---
...
class CortexClient:
    ...
    def ask(
        ...
        """
        Ask a quick chat question (non-streaming).

        For retrieving knowledge from the Cortex, prefer
        ask_streaming(use_agentic=True) — streaming Deep Research.
        The non-streaming endpoint is bounded by a ~28s server deadline
        and rejects use_agentic with 400 agentic_requires_streaming.
...
To retrieve knowledge from the Cortex, start with a streaming Deep Research query — it runs the full agentic multi-step pipeline and streams progress while it works:

```python
# Deep Research (recommended first choice for "ask the Cortex" / "find X in the Cortex")
answer = ""
for event in client.ask_streaming("What are the main findings?", use_agentic=True):
    if 'content' in event:
        answer += event['content']
    elif 'thinking' in event:
        print(f"[research] {event['thinking']}")
print(answer)
```

For quick single-shot chat answers, the non-streaming endpoint works too (bounded by a ~28s server deadline; Deep Research is not available here):
