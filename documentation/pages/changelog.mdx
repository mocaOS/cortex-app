...
## July 4, 2026 (Unit-based usage metering & degraded-document detection)

Two operational features: the monthly quota now measures what actually costs money, and silently broken documents are now visible and fixable from the UI.
...
## July 4, 2026 (Graph builds: leaner extraction & rebuild hardening)

A pass over the knowledge-graph build pipeline focused on API-call economy and robustness on large libraries. Graph content and quality semantics are unchanged; builds complete faster and make substantially fewer LLM calls, especially under provider rate limits.
...
## July 3, 2026 (Step 1: relationship descriptions & batched extraction)
...
## July 3, 2026 (Step 2: targeted relationship discovery)
...
## July 2, 2026 (Reliability: system reset & library import at scale)

Two operations that worked fine on small instances but failed on grown ones are now size-independent.
...
## July 2, 2026 (AskAI v2 — faster Chat & Deep Research)

A structural latency pass over the agentic ask pipeline. A chat turn now costs **2 LLM calls instead of 3** (plus one eliminated helper call per search); on the reference deployment this cut average chat time-to-first-token by **~65%**, and long Deep Research questions that previously hit gateway limits now complete. Answer quality and citation behavior are unchanged — the writer stage is untouched.

**Pipeline (all default-on, each individually revertible via env flag)**
