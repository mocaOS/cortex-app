...
compliance-grade audit trail and a real backup story on the cloud deploy
paths. No API breakage.

### Resilience & data safety

- **Backups on the cloud composes** — the Coolify and Dokploy stacks now
  include the nightly backup sidecar (APOC logical graph export + file-volume
  archive, retention rotation) that previously shipped only with the
