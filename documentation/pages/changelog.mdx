...
- **Chunk-batched relationship extraction validated** — `ENABLE_BATCHED_CHUNK_RELATIONSHIPS=true` (with `RELATIONSHIP_CHUNKS_PER_CALL=4`) packs several chunks into one extraction call: several times fewer Step 1 LLM calls at equivalent extraction quality, which speeds up graph generation considerably when your LLM provider enforces request-rate limits.
