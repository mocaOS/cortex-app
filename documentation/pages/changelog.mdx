...
The API gets a harness-ergonomics pass. Live end-to-end testing of agent integrations (the Hermes/OpenClaw skill, the memory plugin, the MCP server) showed that every external client was re-implementing the same workarounds: downloading the whole corpus because the document list took no query parameters, sniffing SSE frames by which key happens to be present, juggling three different shapes for the same `collection_id`, and treating every question as a cold one-shot because nothing outside the built-in frontend spoke the conversation-memory contract. This release removes those frictions at the source — everything is additive, and no existing integration changes behavior.
...
- **MCP server** — `ask_question` gains a `thread` parameter with the same semantics; the stream aggregator was also fixed to keep reading past the `done` frame when a memory update is still pending.
