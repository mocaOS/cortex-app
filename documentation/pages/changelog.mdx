...
- **D-002 (Low, latent)** — the Compute3/Turbo vLLM-readiness branch called a nonexistent `job.get_auth_token()` and passed a 4th argument to the 3-arg `set_turbo_mode_state`; both now mirror the working live path (`await get_job_token(...)` + 3-arg call).
...
### Testing
- Backend suite expanded from 322 to **458 hermetic tests** (new modules for auth, Compute3, API-usage, vision image-prep, context-curator memory, library-transfer NDJSON round-trip, researcher-agent helpers, skill-service parsing, RRF fusion, and FastAPI endpoint contracts).
