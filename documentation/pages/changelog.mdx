...
- **Web Import redirect chains pre-validated** — server-side redirect chains are now walked hop-by-hop through the same SSRF policy before crawl4ai receives the (final, validated) URL; previously only the initial URL was checked. Legitimate public redirects keep working unchanged.
- **Rate limiting resolves the real client IP behind reverse proxies** — the shipped backend CMDs now run uvicorn with `--proxy-headers`, so per-IP buckets and request logs see the actual client instead of the proxy address. No behavior change unless `RATE_LIMIT_QPM` is set; pin `UVICORN_FORWARDED_ALLOW_IPS` (default `*`) if the backend port is ever published directly.
