...
- **Per-app storage** — a `storage` capability gives an app its own quota-capped, strictly namespaced key/value store (per-app SQLite in the apps volume), usable both from the app's UI and from task steps; it also powers the task engine's built-in fan-out deduplication (`skipIfStored`).
