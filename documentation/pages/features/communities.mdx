...
```bash
curl "http://localhost:8000/api/communities/comm_1" \
  -H "X-API-Key: your-api-key"
```
...
```bash
curl "http://localhost:8000/api/communities/comm_1/documents" \
  -H "X-API-Key: your-api-key"
```
...
```bash
curl -X POST "http://localhost:8000/api/communities/comm_1/summarize" \
  -H "X-API-Key: your-api-key"
```
...
curl -X POST "http://localhost:8000/api/rag" \
  ...
  -H "Content-Type: application/json" \
  -d '{
    "query": "What are the key topics covered in this community?",
    "community_id": "comm_1"
  }'
```
