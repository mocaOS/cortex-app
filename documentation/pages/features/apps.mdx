...
This page covers apps hosted *inside* your instance. The ecosystem also includes **standalone apps** — full applications with their own deployment and users that connect via the REST API with scoped keys; the flagship is [Cortex Chat](/features/cortex-chat), the multi-tenant chat frontend, and [Cortex Trainings](/features/cortex-trainings) turns a collection into interactive training units.
