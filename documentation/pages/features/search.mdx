...
MOCA Library implements **hybrid search** that combines three search methods for optimal results: vector similarity, keyword matching, and knowledge graph traversal.
...
MOCA uses RRF to combine results from multiple search methods:
...
1. **Use collection scoping** - Narrow search to relevant collections
