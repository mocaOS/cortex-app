...
(A single-page import uses a simpler header and is still filed under its domain — with the page title appended when the crawl yields a real one: `# example.com - Page Title` + `> Source: …`, filed as `example.com - Page Title.md`, so repeated one-page imports from the same site stay distinguishable in search results. A URL-style fallback title such as `o.html` keeps the plain `example.com.md`.)
...
docker run -d --name crawl4ai \
  -p 11235:11235 \
  --shm-size=1g \
  -e CRAWL4AI_API_TOKEN=your-shared-token \
...
CRAWL_SERVICE_TOKEN=your-shared-token       # required for crawl4ai >= 0.9.0
```
