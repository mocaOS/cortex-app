...
2. Pick a **provider** (GitHub / GitLab / Gitea). For self-hosted GitLab/Gitea, fill in the API base URL. Its scheme (`http://` or `https://`) is honored for both the REST API and `git clone`, so a plain-HTTP LAN instance works without any TLS allowlist.
...
6. Pick the **collection** the repository's files (and wiki pages) should be filed into. Leave it on *Default collection* to use the instance default. The picker only appears when collections are enabled.
7. Leave the default **"Only ingest `.pdf` and `.md` files"** checked, or uncheck it to set custom filters (see [Filtering](#filtering-what-gets-ingested)).
8. Click **Connect**, then **Sync** to ingest.
...
- **GitLab** — a **Project Access Token** with role **Reporter** and scopes `read_api` + `read_repository` (`read_repository` covers the clone, `read_api` lets Cortex resolve the project, browse your projects and read the wiki). For merge requests, use role **Developer** with `api` + `write_repository`. With a **fine-grained personal access token** (GitLab 18.10+), grant **Code: Download**, **Project: Read** and **Repository: Read** on the repository, plus **Wiki: Read** for the wiki — **User: Read** is not needed; the **Test** button then shows "Token accepted" without a username.
...
- **read_file** — fetch a file's current contents (used before editing it).
- **propose_change** — open a pull request with edits.
- **comment** — comment on an existing pull request.

This is a tool for *changing* the repository, not for reading it. Questions about a connected repo's code or docs are answered from the knowledge base, which already holds its ingested files and returns them as citable sources — so a **read-only** connection deliberately gets no `git_repo` tool at all. Administrators can override that with `RESEARCHER_GIT_TOOL` (`auto` by default, `always`, or `off`).
...
Expand a connection to **Edit** it — change access level, branch, collection, auto-sync interval, filters, wiki ingestion, or rotate the token. Toggling away from the `.pdf`/`.md` default when you have custom filters defined asks for confirmation first.

Changing the **collection** moves every document already synced from that repository into the new collection on save, so a repo never ends up split across two collections. Switching back to *Default collection* only affects files synced from then on; existing documents stay where they are.
