...
|--------|-----------|-------|-------|
| PDF (text-based) | `.pdf` | Milliseconds | anydoc fast path — a full-length book converts in under a second, independent of page count |
| PDF (scanned / image-rich) | `.pdf` | ~1 s/page | Routed to Docling automatically: per-page ML layout analysis + OCR, and figure extraction for vision analysis |
| EPUB | `.epub` | Milliseconds | E-books parsed natively. Kindle formats (`.mobi`, `.azw`) are not supported — convert them to EPUB first (e.g. with Calibre) |
| Plain Text | `.txt` | Instant | Direct text ingestion |
| Markdown | `.md` | Instant | Preserves formatting, headers, code blocks |
| Word | `.docx` | Milliseconds | Parsed natively — upload the `.docx`, not an "Export as PDF" of it |

**Rule of thumb: upload the source format, not a rendering of it.** Text-based
PDFs are fast now, but the source format still preserves structure (headings,
lists, tables) more faithfully than a print rendering, and office-format
embedded images flow into vision analysis on the fast path while PDF images
require the Docling path. Books → EPUB, Office documents → the office file
itself, web content → Web Import or HTML/Markdown. Reserve PDF for content
that only exists as PDF.
...
:::note
**No-content documents:** A file that can never yield content — zero bytes, a PDF with no pages, or a password-protected PDF — finishes as `completed` with `content_status` set to `"empty"` or `"encrypted"` (plus a human-readable `content_note`), **not** as `failed`. Retrying such a file always produces the same result, so failing it would leave permanent noise in the failed list. These documents hold no chunks and never appear in search results; `GET /api/ingestion/status` counts them in a separate `no_content` bucket. The flag is cleared at the start of every processing run, so re-uploading a decrypted or non-empty replacement ingests normally.
:::
...
```bash
# Maximum file size (uploads exceeding this are rejected with HTTP 413)
MAX_FILE_SIZE_MB=50
