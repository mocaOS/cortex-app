...
:::tip{title="Skills for building on Cortex"}

This page is about installing skills **into** Cortex's agent. For the reverse direction — teaching an **external** AI agent (Claude, Cursor, Hermes, or any coding assistant) how to use the Cortex API — see **[Cortex Skills](https://cortexskills.org)** ([source repo](https://github.com/mocaOS/cortex-skills)): curated `SKILL.md` files for every Cortex subsystem, fetchable over HTTP. Point your agent at `https://cortexskills.org/SKILL.md` for the index, or `https://cortexskills.org/llms.txt` for a machine-readable listing.
