...
**Phase B (per-collection, on demand):** Cross-document relationship analysis using the extraction model (instruction-following). Uses a two-phase approach per batch:

1. **Candidate scanning** — The extraction model scans entity pairs with their co-occurring chunk context and proposes candidate relationships.
2. **Structured confirmation** — The extraction model confirms candidates with structured XML output, grounding each relationship in source text.

Entities are grouped into batches using **Union-Find co-occurrence clustering** — entities that share chunks are grouped together, ensuring the LLM sees the most relevant entity pairs. Chunk context is filled dynamically per batch using a **60/40 token split** (60% for chunk text, 40% for entity descriptions and output budget).

Supports **multi-round discovery**: each batch runs multiple rounds (default 3 for initial analysis, 1 for re-analyze) to progressively find relationships, guided by a target **ERR (Entity-Relationship Ratio)** metric (default 1.0). The ERR is also displayed on the Knowledge Graph page as a quality indicator.

Supports two modes: **incremental** (builds on existing relationships) and **rebuild** (deletes all relationships and starts fresh). Relationship types are constrained to 15 standard types via fuzzy matching. A **plaintext fallback parser** handles arrow-format output (e.g., `Entity1 -> RELATION -> Entity2`) when XML parsing fails. Triggered via API or the Knowledge Graph page after document processing.
...
flowchart TD
    ...
    G -.-> G1["Per-collection (extraction model)"]
`} />
...
# RELATIONSHIP_MAX_OUTPUT_TOKENS=16000  # Max OUTPUT tokens for LLM responses
# PARALLEL_RELATIONSHIP_BATCHES=0       # 0 = use CONCURRENT_EXTRACTIONS
