...
Collection scoping applies to all Ask AI modes (standard, streaming, deep research, and fast search) as well as basic search. When a `collection_id` is provided, vector search, keyword search, and graph traversal are all filtered to only include data from that collection's documents.
...
**In the UI:** Open the Settings panel on the Ask AI page and use the **Collection Scope** dropdown to select a collection.
