---
title: Configuration
description: Complete reference for all 160+ environment variables in Cortex
---
...
```bash
# OpenAI-compatible provider (any endpoint that speaks the OpenAI API)
...
OPENAI_MODEL_FAST_MODE=qwen3-6-35b-a3b
...
OPENAI_MAX_CONTEXT=256000       # Default input context budget — sized to the recommended primary's window
```
...
```bash
# Concurrent document processing (2 beats 3 on real providers: concurrent
# long extraction calls share the provider's decode rate and multiply timeouts)
...
# 0 = inherit min(OPENAI_MAX_CONTEXT, 48000) — the inherited value is clamped because
# extraction is decode-bound. Recommended explicit 24000.
# GRAPH_EXTRACTION_MAX_CONTEXT=0

# Output budget for entity-extraction LLM calls
# Default 12000 (≈ half the extraction input budget). Set 0 to inherit OPENAI_MAX_OUTPUT_TOKENS.
...
# (Venice disable_thinking) → sub-second first token.
# On OpenAI GPT-5/o-series, OFF can disable parallel tool calls — set =auto there.
DEFAULT_REASONING_MODE=off

# Deep-research (quality) researcher loop. Default OFF: that loop forces explicit
# reflection through the `reasoning` tool, whose thought persists in the message
# history, while hidden chain-of-thought is discarded between turns and competes
# for the same max_tokens budget. Set =auto to restore provider-default thinking.
...
OUTPUT TOKENS:                          INPUT CONTEXT:
  ...
  (EXTRACTION tier defaults to 12000)     (extraction inherit clamped at 48000)
       ↓                                       ↓
...
OPENAI_MAX_CONTEXT=256000                 # Qwen3.6 35B A3B full input window (= code default)

GRAPH_EXTRACTION_MODEL=qwen3-6-27b    # extraction + (inherited) relationship (256K window)
GRAPH_EXTRACTION_MAX_CONTEXT=24000         # size to the provider's DECODE SPEED, not the model's window — see "Provider Notes: Venice" below
EXTRACTION_MAX_OUTPUT_TOKENS=12000         # ≈ half the input budget — entity-dense docs overflow smaller caps (each overflow split-retries: self-healing but ~2× that batch's wall time)
# RELATIONSHIP_MAX_CONTEXT: leave unset (inherits the extraction budget). Bounded per-call
# output does NOT bound prefill time — wide values (256000) work on fast hosted endpoints
# but prefill for minutes and time out on self-hosted GPUs. Only legacy llm_scan mode
# consumes this budget; the default targeted mode sizes its calls with
# RELATIONSHIP_PAIR_CONTEXT_TOKENS.
...
EMBEDDING_MODEL=text-embedding-3-small     # recommended: OpenAI text-embedding-3-small (1536-dim)
EMBEDDING_DIMENSION=1536
EMBEDDING_API_BASE=https://api.openai.com/v1
EMBEDDING_API_KEY=
EMBEDDING_MAX_INPUT_TOKENS=5400            # default; kept well under the 8192 provider cap because
                                           # providers validate with their OWN tokenizer (counts
                                           # 1.2–1.4× higher than cl100k on punctuation-heavy text —
                                           # see "Provider Notes: Venice")
# Single-provider (Venice-only) alternative: EMBEDDING_MODEL=text-embedding-qwen3-8b +
# EMBEDDING_DIMENSION=4096 (native dim; Neo4j 5.26 supports up to 4096-dim vector
# indexes). Self-hosted vLLM users running Qwen3-Embedding-8B can lift the input cap to 32768.
```
...
```bash
# Adjust extraction-tier output independently — keep it ≈ half of
# GRAPH_EXTRACTION_MAX_CONTEXT (undersizing causes overflow split-retries on
...
```bash
GRAPH_EXTRACTION_MAX_CONTEXT=24000   # right-sized to serving speed (see below)
EXTRACTION_MAX_OUTPUT_TOKENS=12000   # ≈ half the input budget; if timeout-splits appear in the
                                     # logs instead of truncate-splits, lower the input budget —
                                     # the decode rate, not the cap, is then the binding limit
EMBEDDING_MAX_INPUT_TOKENS=5400      # safe under Venice's own token validator
VISION_MAX_CONCURRENT=2              # each in-flight image chains up to 4 API calls
BATCH_PROCESSING_CONCURRENCY=2       # fewer concurrent builds finish FASTER (shared decode rate)
```
...
# Resolve entity names in a query to stored entities (incl. aliases) before
# graph traversal. false = exact-name matching only.
ENABLE_QUERY_ENTITY_RESOLUTION=true

# Collection-scoped vector search: fetch this × the requested candidates from
# the index (cap 200) before filtering, since the index cannot pre-filter. 1 = off.
VECTOR_SCOPED_OVERFETCH=10

# Graph leg: entity-to-entity neighbors + passages ranked by mentioned query
# entities (fast, relevant). false = legacy unranked traversal.
ENABLE_RANKED_GRAPH_TRAVERSAL=true

# Run the vector / keyword / graph legs of one query concurrently. false = sequential.
...
# Candidates kept per knowledge_search after reranking. Also drives the
# per-query fetch depth so the pooled, deduplicated candidate set is about
# twice this value — the reranker selects rather than reorders.
RERANK_TOP_K=15

# Lifecycle (per-instance footprint tuning)
RERANKER_PRELOAD=false          # eager-load at startup; off = lazy, leaner idle instances
RERANKER_IDLE_TTL_SECONDS=0     # 0 = keep resident (default); set >0 to unload after N idle seconds and reclaim ~1 GB
```
...
Text-based PDFs and office documents (Word, Excel, PowerPoint, EPUB) convert through **anydoc** — in-process, no ML models, milliseconds per document — while Docling handles everything that needs layout analysis or OCR: scanned and image-rich PDFs, standalone images, audio, HTML, LaTeX, XML. Files anydoc declines fall through to the Docling paths automatically, so the fast path can only make things faster, never break a conversion.

```bash
ENABLE_ANYDOC=true                    # master switch; false = every conversion uses Docling as before
ANYDOC_PDF_MIN_CHARS_PER_PAGE=200     # PDFs yielding less text per page are treated as scans -> Docling OCR (0 = off)
ANYDOC_PDF_MAX_IMAGES_PER_PAGE=0.5    # with a vision model active: image-denser PDFs keep Docling so figures reach image analysis (negative = off)
```

anydoc cannot extract images from PDFs, which is why image-dense PDFs stay on Docling when vision analysis is on. Embedded images in Word/PowerPoint/EPUB files are extracted on the fast path as usual. A document that converted badly on the fast path can be forced through Docling once: `POST /api/documents/{id}/reprocess?engine=docling`.

### Shared Model Services (cortex-helper)

Offload the cross-encoder and Docling converter to a service hosted once per physical machine (the `cortex-helper` repo), so many tenant stacks on that host don't each load their own copy. Falls back to the built-in local path automatically when unset or unreachable. With the anydoc fast path enabled, only the Docling lane (scans, image-rich PDFs, images, audio) reaches the helper.
...
PROMPT_GUARD_SERVICE_URL=http://cortex-helper:3030  # set = query-time prompt-injection gate via /classify (empty = disabled)
PROMPT_GUARD_THRESHOLD=0.5                       # injection-class probability at/above which a question is refused
DOCLING_CONVERSION_TIMEOUT=600                   # ceiling (s) on a local docling subprocess; on timeout -> document failed (not remote path)
HELPER_SERVICE_TOKEN=                             # shared secret -> X-Helper-Token (reranker, docling, prompt-guard)
```
...
# RESEARCHER_MAX_ITERATIONS_QUALITY=5    # Deep research mode (quality)
...
# Optional dedicated writer model — the researcher keeps OPENAI_MODEL for tool
# calling while a different model composes the final answer. Each property
# falls back to the primary config, so setting just WRITER_MODEL reuses the
# primary gateway and key.
...
# RESEARCHER_GIT_TOOL=auto               # git_repo tool exposure: auto (read/write connections only) | always | off

# Deep research: reflection & convergence (all default on)
# RESEARCHER_FORCE_REFLECTION=true       # Force a reasoning step after search rounds the model didn't reflect on
# RESEARCHER_NOVELTY_MIN_NEW_RATIO=0.35  # Search round below this share of previously-unseen sources counts as stale
# RESEARCHER_NOVELTY_STALE_ROUNDS=2      # Consecutive stale rounds before the answer is written from gathered results
# RESEARCHER_WALL_CLOCK_SECONDS=60      # Research time budget; on expiry the writer starts (0 = unlimited)
```
...
## MCP Server (instance-hosted)

```bash
# Serve the Model Context Protocol from the instance itself at /mcp
# (streamable HTTP, stateless). Off by default — when unset the endpoint
# does not exist (404). Clients connect with the URL + an API key; tool
# calls run through the instance's own REST API, so key permissions,
# collection scoping and quotas apply unchanged.
ENABLE_REMOTE_MCP=true
```

See [MCP Server](/features/mcp-server) for client setup (Claude Code, Cursor, Claude Desktop) and the tool list.

## x402 Payments

Monetize the retrieval endpoints with pay-per-query micropayments via the open [x402 standard](https://github.com/x402-foundation/x402). See the [x402 Payments feature guide](/features/x402-payments) for the full walkthrough.

```bash
# Master switch — and deliberately the ONLY x402 environment variable.
# Everything else (recipient wallet, facilitator URL, network, asset) is
# configured at runtime in Settings → x402 Payments and stored in Neo4j
# (survives redeploys; excluded from library export and system reset).
# Priced API keys activate once that config passes the built-in verification.
X402_ENABLED=true
```
...
Host self-contained web apps (built from the [Cortex App Template](https://github.com/mocaOS/cortex-app-template)) inside your instance. Off by default — when disabled, every app route returns 404 and the admin UI section is hidden entirely. See the [Apps feature guide](/features/apps).

```bash
# Master switch for the Apps subsystem (install / serve / proxy).
# Off by default: zero new surface until an operator opts in.
ENABLE_APPS=true

# Directory holding installed app bundles (relative to project root or
# absolute). Persist via a Docker volume (apps_data) in production.
APPS_DIR=.agents/apps

# Max uploaded app package (zip) size in MB. Uncompressed contents are
# capped at 4x this (zip-bomb guard).
APP_MAX_PACKAGE_MB=50

# Lifetime of the short-lived app tokens handed to sandboxed apps; apps
# renew automatically. The signing secret derives from SESSION_SECRET
# (falls back to ENCRYPTION_KEY / ADMIN_API_KEY).
APP_TOKEN_TTL_SECONDS=900

# Where the app proxy forwards allowlisted /apps/{id}/api/cortex/* calls
# (self-loopback so auth, metering, and rate-limit paths match external
# API traffic).
APP_PROXY_UPSTREAM=http://127.0.0.1:8000

# Timeout (seconds) for the platform "http" capability — server-side
# external calls made on behalf of platform apps, with secrets injected
# from the app's encrypted config.
APP_HTTP_TIMEOUT=30

# --- Platform storage capability (per-app SQLite key/value store) ---

# Per-app storage quota (MB) and per-value size cap (KB).
APP_STORAGE_MAX_MB=50
APP_STORAGE_MAX_VALUE_KB=1024

# --- Platform tasks capability (declarative server-side step-queues) ---

# Max items one task may hold, per-task worker pool ceiling, and the
# global cap on concurrently executing items across ALL apps (protects
# small instances from a fleet of scheduled tasks).
APP_TASK_MAX_ITEMS=2000
APP_TASK_MAX_CONCURRENCY=4
APP_TASKS_GLOBAL_CONCURRENCY=8

# Floor for a scheduled task's everyMinutes interval.
APP_TASK_MIN_SCHEDULE_MINUTES=15
...
APP_TASK_LLM_CALLS_PER_RUN=500

# A step whose output exceeds this (KB) fails — large artifacts belong
# in storage via store steps, not in the step context.
APP_TASK_STEP_OUTPUT_MAX_KB=2048

# Max stored task records per app (finished one-shots age out first).
APP_TASK_MAX_PER_APP=50

# The public app catalog behind the admin "Browse Registry" panel.
# Installs re-verify each artifact against the catalog's pinned sha256
# before unpacking. Set empty to disable registry browsing.
...
```bash
# Master switch for the regex detection + output-filtering + fencing layers
PROMPT_SECURITY=true

# EXPERIMENTAL: ingestion-time document injection scan (flag, never block).
# Disabled and completely hidden by default — opt in explicitly:
...
# Query-time Prompt Guard classifier (shared cortex-helper /classify). Inert unless a URL is set.
PROMPT_GUARD_SERVICE_URL=          # e.g. http://cortex-helper:3030 (empty = disabled)
PROMPT_GUARD_LOCAL=false           # no helper? load the classifier in-process (dev / self-host; needs torch, +RAM)
PROMPT_GUARD=true                  # runtime-toggle default (admin-overridable); +1 query per guarded ask
PROMPT_GUARD_THRESHOLD=0.5         # injection-class probability cutoff (lower = stricter)
```

See the [Security guide](/guides/security) for how the layers fit together.
...
# Deliberately NOT NEXT_PUBLIC_-prefixed: it is read from process.env at runtime
# (server-side), so it takes effect without a frontend rebuild.
...
```bash
MAX_FILES=0              # Max total documents (uploads + custom inputs)
MAX_COLLECTIONS=0        # Max collections (the default collection counts as 1)
MAX_ENTITIES=0           # Max total entities across the graph
MAX_QUERIES_PER_MONTH=0  # Monthly quota, measured in internal LLM completions ("units")
```
...
| `OPENAI_MODEL` | No | `qwen3-6-35b-a3b` | Primary LLM for Q&A, research, chat |
...
| `RESEARCH_REASONING_MODE` | No | `off` | Reasoning mode for the deep-research (quality) researcher loop. Same values as above |
...
| `RESEARCHER_MAX_ITERATIONS_QUALITY` | No | `5` | Agent loop iterations for deep research |
...
| `WRITER_MODEL` | No | `OPENAI_MODEL` | Dedicated model for the answer-writing stage (researcher keeps the primary model) |
| `WRITER_API_BASE` | No | `OPENAI_API_BASE` | API base for the writer model |
| `WRITER_API_KEY` | No | `OPENAI_API_KEY` | API key for the writer model |
...
| `RESEARCHER_GIT_TOOL` | No | `auto` | When the agent gets the `git_repo` tool: `auto` (only when a repo is connected read/write — read-only repos are already ingested and answered from the knowledge base), `always`, or `off` |
...
| `RESEARCHER_WALL_CLOCK_SECONDS` | `60` | Wall-clock budget for the researcher loop (0 = unlimited); on expiry the writer synthesizes from gathered results. Guards against provider queue spikes so the answer always starts; raise or disable for slow self-hosted inference. |
...
| `ENABLE_ANYDOC` | `true` | In-process anydoc fast path for text-based PDFs and office formats (milliseconds, no ML). `false` = every conversion uses the Docling paths as before. |
...
| `ANYDOC_PDF_MAX_IMAGES_PER_PAGE` | `0.5` | With a vision model active: PDFs with more embedded images per page keep the Docling path so figures still reach image analysis. Negative disables. |
