...
Cross-document relationship discovery (Phase B). Run after document processing
to build the relationship graph:

```bash
# Max INPUT context window tokens for relationship analysis batching
# Increase if your main model supports a larger context window
# RELATIONSHIP_MAX_CONTEXT=65536

# Max OUTPUT tokens for relationship analysis LLM responses
# RELATIONSHIP_MAX_OUTPUT_TOKENS=8000

# Number of relationship analysis batches to process in parallel
# Set to 1 for sequential processing, increase for multi-GPU or high-rate-limit setups
# PARALLEL_RELATIONSHIP_BATCHES=1
...
| `RELATIONSHIP_MAX_OUTPUT_TOKENS` | No | `8000` | Max OUTPUT tokens for relationship analysis LLM responses |
| `PARALLEL_RELATIONSHIP_BATCHES` | No | `1` | Parallel relationship analysis batches (1 = sequential) |
