...
# Ingestion-time document injection scan (flag, never block); admin-toggleable at runtime
...
| `INGESTION_INJECTION_SCAN` | No | `true` | Runtime-toggle default for the ingestion-time document injection scan's LLM classifier |
...
| `AUTO_RESUME_PENDING_ON_STARTUP` | `true` | When a restart strands documents mid-processing, automatically restart batch processing for them (quota-guarded). Bulk uploads parked with `start_processing=false` are never auto-started. |
