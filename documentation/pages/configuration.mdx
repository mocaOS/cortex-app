...
# EMBEDDING_MAX_INPUT_TOKENS defaults to 5400 (safe under gateway-side token
# validators — see "Provider Notes: Venice"). Self-hosted vLLM users running
# Qwen3-Embedding-8B can lift to 32768.
```
...
**Concurrency defaults.** The shipped defaults (`BATCH_PROCESSING_CONCURRENCY=3`, `CONCURRENT_EXTRACTIONS=3`, `CONCURRENT_RELATIONS=3`, `VISION_MAX_CONCURRENT=2`) are reliably tested. On providers with limited serving throughput, *lowering* build concurrency often finishes multi-document builds faster — concurrent long extraction calls share the provider's decode rate and can push each other past request timeouts (see "Provider Notes: Venice").
