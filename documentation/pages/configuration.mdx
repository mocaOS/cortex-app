...
| `ENABLE_BATCHED_CHUNK_RELATIONSHIPS` | `false` | Pack several chunks into one per-chunk relationship-extraction LLM call (recommended — several times fewer Step 1 LLM calls at equivalent extraction quality; especially effective under provider rate limits). |
