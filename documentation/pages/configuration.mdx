...
EXTRACTION_MAX_OUTPUT_TOKENS=8000          # extraction output cap (explicit; 0 would inherit the same 8000)
RELATIONSHIP_MAX_CONTEXT=256000            # relationship batching keeps a wide READ window — its output is bounded per call (pairs-per-call cap), unlike extraction
...
EXTRACTION_MAX_OUTPUT_TOKENS=8000    # ≈ what the decode rate delivers inside a request timeout
