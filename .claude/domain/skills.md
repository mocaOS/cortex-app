...
### Response Truncation
`_truncate_response()` intelligently truncates large JSON API responses by progressively slimming array items (truncating string values, flattening nested objects, compacting lists) before falling back to item-level truncation. Every truncated result carries an explicit trailer (`[NOTE: response truncated — … use pagination parameters (e.g. ?limit=, ?page=)]`) so the model knows data was dropped and can paginate, and the tool description teaches pagination upfront. The **writer** copy of a skill-API source is additionally capped at 8KB (`researcher_agent.py` formatted_sources) — the researcher saw the full data; the answer stage doesn't need to pay for it again.

### Prompt hygiene & budget
- `build_activated_skills_block(instructions, max_chars=...)` enforces `MAX_SKILL_INSTRUCTIONS_TOKENS` (~4 chars/token) with an explicit `[skill instructions truncated…]` marker — previously the setting existed but was never applied, so `<active_skills>` grew unbounded with each enabled skill.
- Researcher prompts are parameterized on `has_skills`: skill-less deployments get prompts with **no** `http_request`/`<active_skills>` references (they used to invite hallucinated tool calls that burned an iteration). The speed-mode skill guidance is now conditional: after a skill call the model searches the KB only if the question also needs document context, otherwise it calls `done` (was: unconditional follow-up search).
- Skill state is loaded off the event loop (`_load_skill_state` via `asyncio.to_thread`), and `get_skill_config_schema`/`get_skill_base_url` are TTL-cached (`_get_skill_node_cached`) — previously 2 uncached Neo4j round-trips × N skills × every ask request, on the event loop.
