...
**File-restore hardening (crafted-archive defense)**: the archive is admin-supplied, so `documents.ndjson` `file_path` values are untrusted. `_safe_import_basename` rejects empty/`.`/`..` basenames (a `..` basename previously remapped to `uploads/..` and aborted the whole import with `IsADirectoryError`) — the document *node* still imports, file-less, with a warning. On-disk names are doc-scoped (`{sanitized_doc_id}_{basename}` via `_doc_scoped_filename`), so two documents sharing a basename can no longer silently overwrite each other and serve one doc's bytes as the other's; the restore additionally re-proves the destination is contained in the upload/custom-inputs dir and wraps each copy so one bad entry degrades to a warning instead of failing the import.
