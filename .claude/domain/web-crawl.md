...
There is no crawl-history surface to leak; 1–3 make cross-tenant visibility impossible, 4 controls who can call it. **SSRF**: every URL is validated through `ssrf_guard` under `WEB_IMPORT_ALLOW_PRIVATE` (default `true` — deliberate, so intranet crawls keep working; loopback/link-local/metadata always refused). Because crawl4ai follows redirects inside its own container — where cortex-app cannot intercept them — both endpoints first walk each URL's **server-side 30x chain** themselves (`crawl_client.resolve_redirect_chain`: ≤5 hops, per-hop guard validation, cycle detection, ~15s total budget; HEAD with streamed-GET fallback) and hand crawl4ai the **final, validated URL** to fetch directly. A hop to a blocked target is refused with 400 before anything is queued; probe *transport* failures fail open to the last validated URL so the thin pre-flight client never breaks imports of pages only crawl4ai's real browser can render. **Residual**: JS/meta-refresh redirects and target behavior changing between probe and fetch (TOCTOU) remain crawl4ai-side — on shared hosts the complete answer is still network-layer egress rules blocking RFC1918 + `169.254.169.254` on the crawl4ai container (see `cortex-helper/README.md`).
