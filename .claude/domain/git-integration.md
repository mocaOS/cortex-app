...
**Egress policy.** Provider REST calls go through `ssrf_guard.async_request_hook(allow_private=settings.git_http_allow_private, allowlist=GIT_HTTP_INSECURE_HOSTS)` — every redirect hop re-validated, loopback/link-local/metadata always refused. `GIT_HTTP_ALLOW_PRIVATE` defaults **true** (a self-hosted forge on an intranet is the legitimate case) but is **false in `dokploy/`**, i.e. on every cloud tenant, where a private-range `base_url` would be a neighbouring stack on the shared docker network. Two caveats: `GIT_HTTP_INSECURE_HOSTS` doubles as the guard's bypass allowlist (operator-set, never user-reachable), and the **clone/fetch subprocess is NOT behind the guard** — only the REST surface is, so container-level egress filtering is the only complete answer for the clone path.
