...
- **Batched relationship writes**: with `ENABLE_BATCHED_KG_WRITES`, per-chunk relationships are stored one UNWIND call per completed chunk (`store_relationships_batch`, APOC with per-item fallback) instead of one round trip per relationship.
