...
- **Reasoning suppressed, in both modes** (`_writer_reasoning_mode`) — the writer only composes prose from context the researcher already gathered, so hidden reasoning buys nothing, and on thinking models the trace is billed against the *same* `max_tokens` budget as the visible answer. Measured live on `qwen3-6-35b-a3b` (quality, 15 sources, 4000 cap): the trace ran 2.7k–4.3k tokens and the answer came back cut mid-word — or completely empty when the trace alone exhausted the cap. Note the researcher loop keeps `AUTO` in quality mode (`_chat_reasoning_mode`); the forced-reflection micro-call depends on the model actually thinking. Don't collapse the two helpers back together.
