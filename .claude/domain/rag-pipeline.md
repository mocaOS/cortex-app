...
- **Per-key rate limiting** (`RATE_LIMIT_QPM`, 0=off): token bucket on ask/upload endpoints, 429 + `Retry-After`. The per-IP fallback bucket keys on the real client IP — the shipped CMDs run uvicorn `--proxy-headers` (see `UVICORN_FORWARDED_ALLOW_IPS` in [`environment.md`](../environment.md)).
