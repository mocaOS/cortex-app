...
**Concurrency cap on auto-started pipelines**: every individually-started pipeline (API upload with `start_processing=true`, text ingestion, single reprocess) funnels through `_process_document_with_cleanup`, gated by a global semaphore sized `BATCH_PROCESSING_CONCURRENCY` (`_get_processing_slots`). A burst of API ingests therefore queues instead of launching one pipeline per document. A doc that has to wait is marked `processing` with progress message "Queued — waiting for a free processing slot" — this keeps `process_pending_documents` and the stranded-doc sweep from grabbing it (it has a live task), and on restart the boot resume resets/resumes it like any stranded `processing` doc. It additionally carries the additive `processing_queued` flag (`set_document_queued_state`, mirroring `processing_paused`; cleared on slot acquisition and by both stranded resets) so the UI counts it as **Queued**, not Processing — a 300-doc sync-app burst reads "6 processing, 294 queued" instead of 300 concurrent pipelines (2026-07-21; extract-page tile + DocumentCard badge key on the flag). Batch processing (`process_pending_documents`) keeps its own semaphore over the same setting (explicit `concurrency` query param can override it per call). Tests: `backend/tests/test_processing_slots.py`.

## Conversion (anydoc fast path → Docling)

**Conversion routing** (`_convert_document_subprocess`), in order:

1. **anydoc fast path** (`services/anydoc_converter.py`, since 2026-08-06, `ENABLE_ANYDOC` default true): office formats + EPUB + text-based PDFs convert **in-process** via the pure-Rust `firecrawl-anydoc` wheel — milliseconds and ~41 MB transient RSS instead of docling's per-page layout inference (462-page book: 0.47 s vs ~590 s, which used to hit `DOCLING_CONVERSION_TIMEOUT`). No subprocess, no conversion-slot semaphore, no page chunking on this path. The gate declines — returning the file to the docling paths below — on: ineligible extension (html/images/audio/tex/xml), typed anydoc errors (scanned PDF → `UnsupportedError: OCR is required`, encrypted, malformed), low text yield (`ANYDOC_PDF_MIN_CHARS_PER_PAGE`, catches hybrid scans), or image-rich PDFs while a vision model is active (`ANYDOC_PDF_MAX_IMAGES_PER_PAGE`, counted via pypdf) — anydoc has **no document model for PDFs, so PDF images are invisible to it**; image-rich PDFs stay on docling so figures keep reaching vision analysis. Office-format embedded images ARE extracted (anydoc `to_document()` assets → the same images contract, `page_number`/`bbox` = None). `engine="docling"` skips the fast path: used by image re-extraction (`resume_image_analysis`) and forced reprocesses (`POST /api/documents/{id}/reprocess?engine=docling`, which also bypasses delta-skip and chunk reuse). Engine identity is part of `_reprocess_config_hash`. Tests: `backend/tests/test_anydoc_router.py`.
2. **Shared helper**: when `DOCLING_SERVICE_URL` is set, the file is POSTed to the shared `cortex-helper` `/convert` service (warm converter, ~0.04 s vs ~4.5 s for a cold subprocess that reloads OCR/layout models every call), with automatic fallback to the local subprocess if the service is unreachable (unless `HELPER_STRICT_REMOTE`).
3. **Local subprocess**: `docling_worker.py` runs as a separate process for CPU-bound ML inference, OCR, and table structure recognition with memory optimizations for large documents. Images are extracted during this phase for later vision analysis. (EPUB parses natively as XHTML with no per-page layout ML — though EPUBs now normally take the anydoc path anyway.)

**Scanned-PDF OCR retry (2026-08-06)**: vision-enabled instances run docling with `do_ocr = not use_vision` — but a scan's text lines are layout-classified as TEXT regions (not pictures), so with no text layer and no OCR the conversion returns neither markdown nor images and the doc used to fail silently as "No content extracted" (pre-existing bug, surfaced by the router deliberately sending scans to docling). `_convert_document_subprocess` now retries such a PDF exactly once through `_convert_document_docling(force_ocr=True)`; `force_ocr` rides the worker stdin protocol and the helper `/convert` form field (helper warm-converter cache keyed by `(use_vision, force_ocr)`; an old helper ignores the field → fails as before, never worse — **helper deploy required for the fix**). Born-digital docs never hit the retry (they have text or pictures). `resume_image_analysis` picks its engine per extension: `.pdf` forces docling (anydoc has no PDF images — a legacy doc's counters would falsely reconcile), office formats resume through the router so image order matches the original anydoc conversion (image chunk ids are index-based). Tests: `test_anydoc_router.py` (TestOcrRetry), cortex-helper `tests/test_convert_endpoint.py`.

The in-process `DocumentProcessor` converter (`_build_docling_converter` / `_get_converter`) is **lazy** — docling is NOT imported at module scope (that would pull torch + docling-ibm-models ~244 MB into every backend at startup); it builds on first call, which the live subprocess/service path never triggers. See [`environment.md`](../environment.md#anydoc-fast-path-conversion) and [`environment.md`](../environment.md#shared-model-services-cortex-helper).
