...
- `services/remote_mcp.py` — instance-hosted MCP (`ENABLE_REMOTE_MCP`): hand-rolled stateless streamable-HTTP transport at `/mcp`; tools dispatch through the instance's own REST API via an in-process ASGI client, so auth/scoping/quotas apply unchanged. See [`.claude/domain/rag-pipeline.md`](domain/rag-pipeline.md)
