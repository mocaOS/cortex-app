...
- `services/error_tracking.py` — GlitchTip/Sentry error tracking: `init_sentry()` (called in `main.py` before app construction + by the docling worker). Env-driven (`SENTRY_DSN`); no-op when unconfigured. Events tagged `service` + `request_id`. Frontend counterpart: `@sentry/nextjs` (instrumentation files + `withSentryConfig` source-map upload). See [`.claude/domain/observability.md`](domain/observability.md#error-tracking-glitchtip)
...
- `services/session_service.py` — server-side sessions (`ENABLE_SESSIONS`): per-key conversation state (history + curated memory blob) with summarized_count-preserving trim. See [`.claude/domain/rag-pipeline.md`](domain/rag-pipeline.md)
- `services/remote_mcp.py` — instance-hosted MCP (`ENABLE_REMOTE_MCP`): hand-rolled stateless streamable-HTTP transport at `/mcp` (the frontend rewrites `/mcp` to the backend, so the UI domain serves it too on split-domain deployments); tools dispatch through the instance's own REST API via an in-process ASGI client, so auth/scoping/quotas apply unchanged. See [`.claude/domain/rag-pipeline.md`](domain/rag-pipeline.md)
- `services/webhook_service.py` — outbound webhooks (`ENABLE_WEBHOOKS`): admin-managed endpoints, HMAC-signed fire-and-forget delivery with retries for document/task lifecycle events. See [`.claude/domain/document-pipeline.md`](domain/document-pipeline.md#ingestion-status--webhooks)
