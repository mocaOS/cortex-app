...
| `bench/qa_evaluator.py` | Q+A retrieval evaluation. `generate_question_bank()` runs once per batch from `bench/files/`. `run_question_set(cx, q, mode=…)` posts speed to non-streaming `/api/ask` and quality to `/api/ask/stream` via `cx.ask_deep_research()` (SSE aggregated — the non-streaming endpoint rejects `use_agentic=true` with 400). `judge_answers()` is one end-of-batch LLM call scoring every (run × mode × question) on faithfulness/completeness/groundedness/conciseness 1–5. `apply_qa_scores_to_runs()` merges aggregates back. |
...
2. **Per-combo evaluation** (`run_question_set`) — runs after Step 3 inside `run_combo`, in two phases:
   ...
   - `qa_quality`: same bank → `POST /api/ask/stream` with `use_agentic=True`, aggregated from the SSE frames by `cx.ask_deep_research()` (the non-streaming `/api/ask` rejects agentic with `400 agentic_requires_streaming`). Researcher agent loop. 300 s per-question timeout.
