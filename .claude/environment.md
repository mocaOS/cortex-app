...
- `RATE_LIMIT_QPM` (default: 0 = off) + `RATE_LIMIT_BURST` (default: 10) — per-API-key token-bucket guardrail on ask/upload endpoints (429 + `Retry-After`). Billing remains `MAX_QUERIES_PER_MONTH`. The per-IP fallback bucket keys on `request.client.host`, which the shipped Docker CMDs set from `X-Forwarded-For` (uvicorn `--proxy-headers`) — see `UVICORN_FORWARDED_ALLOW_IPS`.
- `UVICORN_FORWARDED_ALLOW_IPS` (default: `*` in the shipped Docker CMDs) — which peers uvicorn trusts `X-Forwarded-For` from (consumed by the Dockerfile CMD, not the app). `*` is safe in every shipped stack (the backend port is never published — only the proxy is), and means uvicorn takes the *leftmost* XFF entry. Operators who publish the backend port directly should pin their proxy subnet(s) (comma-separated IPs/CIDRs) — uvicorn then walks the chain right-to-left for the first *untrusted* entry, which also makes client-injected XFF prefixes unspoofable.
- `RESEARCHER_WALL_CLOCK_SECONDS` (default: 120, 0 = unlimited) — wall-clock budget for the researcher loop; on expiry the writer synthesizes from what was gathered. Healthy deep-research gathering takes 30-60s; the budget guards against provider queue spikes (measured on Venice: 107s for a 2-token completion) so the answer always starts. Checked between iterations (one in-flight call can overshoot); when <20s remain the forced-reflection micro-call is skipped. Raise or disable for slow self-hosted inference.
...
- `WEB_IMPORT_ALLOW_PRIVATE` (default: `true`) — SSRF policy for Web Import URLs, applied to the initial URL **and every server-side redirect hop** (the endpoints pre-walk the 30x chain via `crawl_client.resolve_redirect_chain` and hand crawl4ai the final validated URL; JS-side redirects remain a crawl4ai-side residual covered by network egress rules — see `domain/web-crawl.md`). Loopback/link-local/metadata are always blocked; private (RFC1918/ULA) ranges are allowed by default so intranet crawls keep working. Set `false` to also block private targets. Git provider `base_url` uses the same guard (private allowed — self-hosted git on an internal IP is legitimate — with `GIT_HTTP_INSECURE_HOSTS` as its allowlist).
...
- `TORCH_VARIANT` (backend `Dockerfile.prod` build `ARG`, default: `cuda`) — **not** a runtime env var an operator sets in `.env`; listed here only because the published self-host backend image is already built with `TORCH_VARIANT=cpu` (smaller, no CUDA wheels — every containerized deploy runs Docling's CPU path anyway regardless of this flag). Nothing for a self-hoster to configure.
