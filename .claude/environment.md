...
**Input context chain:** `RELATIONSHIP_MAX_CONTEXT` → `GRAPH_EXTRACTION_MAX_CONTEXT` → `OPENAI_MAX_CONTEXT=256000` (extraction inherit clamped at 48000; `EXTRACTION_MAX_OUTPUT_TOKENS` ships 12000 instead of 0-inherit)
...
- `INGESTION_INJECTION_SCAN` (default: true) — **default** for the ingestion-time prompt-injection scan's LLM classifier; admin-overridable at runtime (persisted as `SystemMeta` key `setting:ingestion_injection_scan`, read via `neo4j.get_runtime_setting`). Off → free heuristic only (no per-doc query). The free heuristic runs regardless. See `.claude/domain/document-pipeline.md`.
