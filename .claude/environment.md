...
`BATCH_PROCESSING_CONCURRENCY` also caps **individually-started** pipelines (API upload with `start_processing=true`, text ingestion, single reprocess): they queue on a global semaphore (`_get_processing_slots` in `document_processor.py`), so a burst of API ingests never runs more than this many pipelines at once. While waiting, a doc is marked `processing` with a "Queued — waiting for a free processing slot" message (keeps the pending-batch pickup and stranded-doc sweep off it; restart-safe via boot resume).
