...
- `DEFAULT_REASONING_MODE` (default `off`) — the **chat/answer path**: the speed-mode researcher loop + the answer writer (`researcher_agent.py`) and the non-agentic streaming writer + fast-search path (`main.py`), all routed through `safe_chat_completion`. Default `off` because on reasoning-capable models (esp. Venice) hidden chain-of-thought streams in a separate `reasoning_content` channel — adding 3–14s before the first answer token and, across the multi-iteration agent loop, frequently exhausting the budget into **empty/timeout answers**. `off` (Venice `disable_thinking`) cuts model time-to-first-token to <1s. Deep-research (`quality`) mode reads `RESEARCH_REASONING_MODE` instead. Caveat: on **OpenAI** GPT-5/o-series, `off` maps to `reasoning_effort` `none`/`minimal`, which can disable parallel tool calls in the agent loop — OpenAI-backed operators who rely on that can set `DEFAULT_REASONING_MODE=auto` to restore provider-default thinking on chat.
- `RESEARCH_REASONING_MODE` (default `off` since 2026-08-15; was hardcoded AUTO) — the **deep-research (`quality`) researcher loop** in `researcher_agent.py` (`_chat_reasoning_mode`). Off because deliberation in that loop is *explicit*: the `reasoning` tool, which the forced-reflection micro-call pins `tool_choice` to, so the thought is produced with or without hidden thinking — and unlike hidden chain-of-thought (discarded between chat-completion turns, re-paid every iteration) it persists in the message history and steers the next round. Hidden reasoning also shares the `max_tokens` budget with visible output: thinking-by-default models (Qwen3.8 ships `reasoning_effort=xhigh`) can spend the entire cap on a trace. Set `auto` to restore provider-default thinking if a model's tool-calling regresses without it; keep `RESEARCHER_FORCE_REFLECTION` on, since it becomes the only deliberation mechanism. The writer stays OFF in both modes regardless (`_writer_reasoning_mode`).
- `REASONING_MODEL_OVERRIDES` (default empty) — escape hatch for novel models the heuristics get wrong. Format: `model1:mode1,model2:mode2`. Example: `gpt-5.8:none,custom-llm:minimal`. Applies to all five modes above (extraction, relationship, vision, default, research).
...
`BATCH_PROCESSING_CONCURRENCY` also caps **individually-started** pipelines (API upload with `start_processing=true`, text ingestion, single reprocess): they queue on a global semaphore (`_get_processing_slots` in `document_processor.py`), so a burst of API ingests never runs more than this many pipelines at once. While waiting, a doc is marked `processing` with a "Queued — waiting for a free processing slot" message plus the additive `processing_queued` flag, which the UI renders as **Queued** rather than Processing (keeps the pending-batch pickup and stranded-doc sweep off it; restart-safe via boot resume).
...
- `ENABLE_INGEST_RESUME` (default: **true**) — mid-document checkpoint/resume for Step 1: stored chunks+embeddings are reused when the file+config fingerprint matches, entity extraction persists per-batch with a chunk-range watermark (`Document.extraction_done_ranges`), and per-chunk relationship completion is flagged (`Chunk.rels_extracted`). An interrupted run (restart, endpoint outage) resumes where it stopped instead of re-paying Docling + embeddings + all extraction batches. See [`domain/document-pipeline.md`](domain/document-pipeline.md#ingest-checkpointresume--endpoint-outage-pause).
- `LLM_OUTAGE_MAX_WAIT_SECONDS` (default: 900) — when the extraction/relationship endpoint refuses connections mid-document (litellm/gateway restart), batches are requeued and re-probed with backoff up to this budget instead of being silently dropped; past it the document fails with its checkpoint intact.
...
- `RESEARCHER_WALL_CLOCK_SECONDS` (default: 60, 0 = unlimited) — wall-clock budget for the researcher loop; on expiry the writer synthesizes from what was gathered. Healthy deep-research gathering takes 30-60s; the budget guards against provider queue spikes (measured on Venice: 107s for a 2-token completion) so the answer always starts. Checked between iterations (one in-flight call can overshoot); when <20s remain the forced-reflection micro-call is skipped. Raise or disable for slow self-hosted inference.
- `RERANK_TOP_K` (default: 15) — candidates kept per knowledge_search after reranking; since 2026-09 it also derives the per-query fetch depth so the pooled, chunk-deduped candidate set is ~2× this value (`_per_query_candidate_k`: 3 queries → 10 each, floor 5, cap 12). Lower on remote rerankers to trade recall for latency.
- `ENABLE_QUERY_ENTITY_RESOLUTION` (default: true) — resolve query-side entity mentions onto stored `Entity.name` values (exact → case-insensitive name/alias → fulltext on the name field) before graph traversal in `hybrid_search_rrf`; 60s in-process cache per name set. `false` → raw exact-name matching (pre-2026-09; graph leg mostly silent). See [`domain/entities.md`](domain/entities.md#query-time-resolution).
- `VECTOR_SCOPED_OVERFETCH` (default: 10) — when a collection/filter scope applies, `vector_search` asks the ANN index for this × top_k candidates (cap 200) before post-filtering (Neo4j 5.x vector indexes cannot pre-filter). `1` = off (legacy: scoped searches returned a fraction of top_k). Unscoped searches unaffected.
- `ENABLE_RANKED_GRAPH_TRAVERSAL` (default: true) — graph leg of hybrid search uses `traverse_for_retrieval` (Entity-only neighbor rings, chunks ranked by mentioned query entities, LIMIT in Cypher; ~10× faster on hubs). `false` → legacy `traverse_from_entities` (co-mention paths, arbitrary 10 chunks). See [`domain/entities.md`](domain/entities.md#entity-traversal-constraint).
- `ENABLE_PARALLEL_SEARCH_LEGS` (default: true) — run the vector / keyword / graph legs of one hybrid query concurrently (`_search_leg_executor`, 16 threads). `false` → sequential legs; use on memory-starved tenant Neo4j containers if concurrent query load bites.
...
- `RESEARCHER_MAX_ITERATIONS_SPEED` (default: 3), `RESEARCHER_MAX_ITERATIONS_QUALITY` (default: 5) — agent loop iteration caps
...
- `WRITER_MODEL`, `WRITER_API_BASE`, `WRITER_API_KEY` (default: empty = primary) — optional dedicated model for the writer stage (agent pipeline Phase 3 in `researcher_agent.py` + the legacy non-agent streaming writer in `main.py`). The researcher loop needs strong tool calling; the writer only composes prose from already-gathered context — this lets a better prose stylist (or a cheaper model) handle synthesis without touching the researcher. Per-property fallback to the primary config (`get_writer_llm_config()` in `llm_config.py`, same idiom as the extraction/relationship tiers), so setting just `WRITER_MODEL` reuses the primary gateway + key. Writer reasoning stays suppressed regardless of model.
...
- `RESEARCHER_FORCE_REFLECTION` (default: true) — quality mode: after a search round with no reflection (neither a `reasoning` call nor prose alongside the tool calls), one micro-call with `tool_choice` pinned to `reasoning` records the model's analysis in the history and steers the next round (models that skip reflection — observed on qwen3.6-a3b — otherwise degrade into shotgun query volleys; gemma4 reflects on its own and never triggers it). Disarmed per-run when the provider rejects named tool_choice.
- `RESEARCHER_GIT_TOOL` (default: `auto`) — when the researcher gets the `git_repo` tool. `auto`: only when a `read_write` git connection exists (writes are the tool's reason to exist; a read-only connection's files are already ingested, so `knowledge_search` serves them as citable sources whereas `git_repo` reads land in researcher-only messages the writer never sees). `always`: any connection (legacy). `off`: never — ingestion unaffected. When exposed, a `<connected_repository>` prompt block names the repo and forbids exploratory `read_file` (the tool cannot list or search paths, so exploration = guessing). See [`domain/git-integration.md`](domain/git-integration.md#write-tool-git_repo-researcher-agent).
- `RESEARCHER_NOVELTY_MIN_NEW_RATIO` (default: 0.35), `RESEARCHER_NOVELTY_STALE_ROUNDS` (default: 2) — convergence stop: when this many consecutive search rounds each return fewer than the ratio of previously-unseen chunks, the loop breaks to the writer instead of burning remaining iterations (which only dilute the writer context). The per-iteration system note also reports last-round novelty to the model so it can call `done` itself. 0 disables.
...
- `SKILL_HTTP_ALLOW_PRIVATE` (default: `false`) — SSRF policy for all skill-originated egress: the agent `http_request` tool (whose URL is LLM-chosen and steerable via prompt injection in ingested content), legacy `tools.json` HTTP tool execution, and skill install-from-URL. By default the target is resolved and requests to private/loopback/link-local/metadata (`169.254.169.254`) addresses are blocked (`services/ssrf_guard.py`; `http_request` additionally re-validates every redirect hop via an httpx event hook). Set `true` only if skills legitimately call APIs on private IPs.
- `SKILL_HTTP_ALLOWED_HOSTS` (default: empty) — comma-separated hostnames exempt from the skill-egress SSRF check (explicit opt-in for a specific self-hosted skill API on an internal host). Union'd with `SKILL_HTTP_INSECURE_HOSTS` for the `http_request` tool.
...
- `ADMIN_EMAIL`, `ADMIN_PASSWORD`, `ADMIN_API_KEY`, `SESSION_SECRET` — admin auth. Login validation happens in the Next.js frontend (`lib/auth.ts` consumes `ADMIN_EMAIL`/`ADMIN_PASSWORD`/`SESSION_SECRET`); the backend consumes `ADMIN_API_KEY` (and checks `ADMIN_PASSWORD`/`SESSION_SECRET` only in the production-hardening validator). `lib/auth.ts` has **no code-level fallback** for `ADMIN_EMAIL`/`ADMIN_PASSWORD`: either missing → login answers "Admin authentication not configured" + a boot `console.warn` naming the missing var(s) (a silent `admin@example.com` default used to mask env-file load failures as "Invalid email or password"). The compose files still default `ADMIN_EMAIL` at the env layer (`${ADMIN_EMAIL:-admin@example.com}`), where the substitution is visible. In `ENVIRONMENT=production`, startup fails fast if `SESSION_SECRET` is < 32 chars while `ADMIN_PASSWORD` is set (see [Deployment & CORS](#deployment--cors)).
- `SESSION_COOKIE_SECURE` (default: unset = `Secure` in production builds) — runtime override for the admin session cookie's `Secure` flag (`lib/session.ts:resolveCookieSecure`, accepts `true`/`false`/`1`/`0`). Exists because `NODE_ENV` is inlined at build time by Next.js, so the flag was otherwise baked into the image; browsers silently drop `Secure` cookies over plain HTTP, making login on a TLS-less LAN self-host appear to do nothing. Mapped through the frontend service as `${SESSION_COOKIE_SECURE:-}` (empty = default) in `docker-compose.yml`, `docker-compose.prod.yml`, `coolify/docker-compose.coolify.yml` and `dokploy/docker-compose.dokploy.yml`. The self-host stack (`selfhost/`) is a deliberate exception: its base `docker-compose.yml` omits this var entirely — that absence IS the domain-mode security fix, since unset falls through to the frontend's own Secure-in-production default under HTTPS — while its localhost overlay (`docker-compose.ports.yml`) maps it to `${SESSION_COOKIE_SECURE:-false}` so plain-HTTP admin login keeps working there. Do **not** "restore consistency" by adding this var back to `selfhost/docker-compose.yml`'s base file — that would reintroduce a non-Secure session cookie on every public HTTPS self-host install.
...
- `ENABLE_SESSIONS` (default: false) — server-stored conversation state for API consumers (`POST /api/sessions` + `session_id` on ask). Off by default because enabling means the instance persists conversation content on behalf of callers — an explicit operator data-retention decision. Tuning: `SESSION_MAX_TURNS` (200), `SESSION_TTL_DAYS` (30, 0=never), `SESSION_MAX_PER_KEY` (500).
- `ENABLE_REMOTE_MCP` (default: false) — serve the Model Context Protocol directly from the instance at `/mcp` (stateless streamable HTTP). Claude Desktop/Code, Cursor etc. connect with the URL + an API key — no npm install. Off = `/mcp` 404s (zero traces). See `app/services/remote_mcp.py`.
...
Env surface specific to the `selfhost/` Compose stack (`.env.example` documents the full operator-facing set; see [`.claude/development.md`](development.md) for the stack itself). A few vars that land here are worth calling out because they don't fit neatly elsewhere:

- `LOGO_URL` (default: empty → `/logo.svg`) — runtime-read logo override (`frontend/src/app/layout.tsx`, `components/layout/Header.tsx`), added alongside the self-host work. Same non-`NEXT_PUBLIC_` rationale as the pre-existing `ACCENT_COLOR`: read from `process.env` per-request rather than inlined at build time, so one prebuilt image can be rebranded per deployment without a rebuild. `NEXT_PUBLIC_LOGO_URL` still works as a build-time fallback for older deploys.
- `TORCH_VARIANT` (backend `Dockerfile` + `Dockerfile.prod` build `ARG`, default: `cuda`; **both compose files override to `cpu`**) — **not** a runtime env var an operator sets in `.env` for prebuilt images; the published self-host backend image is already built with `TORCH_VARIANT=cpu` (smaller, no CUDA wheels — every containerized deploy runs Docling's CPU path anyway regardless of this flag). Source builds via `docker-compose.yml`/`docker-compose.prod.yml` also default to `cpu` (~2.5GB smaller image); set `TORCH_VARIANT=cuda` in `.env` only on a GPU server.
- `SENTRY_DSN_BACKEND`, `SENTRY_DSN_FRONTEND`, `CHAT_SENTRY_DISABLED` — the self-host stack **inverts** the "Shipped defaults" rule under [Error Tracking (GlitchTip)](#error-tracking-glitchtip) above: `selfhost/docker-compose.yml` maps `SENTRY_DSN_BACKEND`/`SENTRY_DSN_FRONTEND` with the no-colon empty-default form (`${SENTRY_DSN_BACKEND-}` etc.), so error tracking is **off** unless an operator opts in with their own DSN — self-hosters' stack traces must never reach the maintainers uninvited. `CHAT_SENTRY_DISABLED` (default `1`) only silences cortex-chat's **server-side** reporting; the client half is disabled separately at cortex-chat's image-build time (`NEXT_PUBLIC_SENTRY_DISABLED=1`, baked in by its own release workflow), since Next.js inlines `NEXT_PUBLIC_*` into the browser bundle and no runtime env var can reach code already inlined there.
