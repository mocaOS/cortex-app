...
- Multi-format document support via Docling (PDF, DOCX, PPTX, XLSX, HTML, MD, images)
"""
...
# Docling for multi-format document conversion
...
class DocumentProcessor:
    ...
    def __init__(self):
        ...
        # Docling handles: PDF, DOCX, PPTX, XLSX, HTML, MD, images, and more
        self.docling_converter = DocumentConverter()
        logger.info(
            "Docling converter initialized for multi-format document processing"
        )
        ...
        logger.info(
            f"Document processor initialized (GraphRAG: {self.graph_extractor.is_available})"
        )
    ...
    def _is_supported_format(self, file_type: str) -> bool:
        ...
        Docling supports: PDF, DOCX, PPTX, XLSX, HTML, MD, images (PNG, TIFF, JPEG, etc.)
        """
    ...
    async def _process_document(self, doc_id: str, file_path: str, file_type: str):
        ...
        try:
            ...
            # Convert file using Docling (supports PDF, DOCX, PPTX, XLSX, HTML, MD, images)
            # Run conversion in thread pool (CPU-bound)
            result = await loop.run_in_executor(
                _get_processing_executor(),
                functools.partial(
                    self.docling_converter.convert, source=Path(file_path)
                ),
            )
            ...
            # Create a Haystack document from the markdown content
            documents = [
                HaystackDocument(
            ...
            logger.info(
                f"Successfully converted {file_type} file to markdown ({len(markdown_content)} chars)"
            )
