...
import type { WebContentFilter, WebDiscoverLink, TaskProgress } from "@/types";

interface WebImportModalProps {
  isOpen: boolean;
  onClose: () => void;
  /** Called once the import task completes, so the document list can refresh. */
  onImported?: () => void;
}
...
export default function WebImportModal({ isOpen, onClose, onImported }: WebImportModalProps) {
  ...
  // Close on Escape — but never mid-import (we want the user to see completion).
  useEffect(() => {
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      setUrlsText("");
      // Refresh the document list behind the modal so the new files are there
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative w-full max-w-2xl mx-4 max-h-[90vh] flex flex-col bg-card border border-border rounded-2xl shadow-2xl">
        {/* Header */}
        <div className="flex items-center justify-between p-6 border-b border-border shrink-0">
          <div className="flex items-center gap-2">
            <Globe className="w-5 h-5 text-accent" />
            <h2 className="text-lg font-semibold text-foreground">Web Import</h2>
            <span className="text-xs text-muted-foreground">powered by Crawl4ai</span>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="p-6 space-y-6 overflow-y-auto">
          {/* Completion state takes over the body */}
          {result ? (
            <div className="space-y-4">
              ...
              {result.failures.length > 0 && (
                <div className="space-y-2">
                  ...
                  >
                    ...
                    {result.failures.length} failed {result.failures.length === 1 ? "URL" : "URLs"}
                  </button>
                      ...
                      >
                        <div className="space-y-1 max-h-48 overflow-y-auto pr-1">
                          {result.failures.map((f, i) => (
                            <div key={`${f.url}-${i}`} className="rounded-lg bg-muted/50 px-3 py-2">
              ...
              <div className="flex gap-3">
                ...
                <button
                  type="button"
                  onClick={onClose}
                  className="flex-1 px-4 py-3 rounded-xl bg-accent text-accent-foreground text-sm font-medium hover:bg-accent/90 transition-colors"
                >
                  Done
                </button>
              </div>
            ...
            <>
              ...
              <div className="space-y-2">
                ...
                <textarea
                  ...
                  rows={5}
                  disabled={isImporting}
                  className="w-full px-4 py-3 bg-background border border-border rounded-xl text-foreground placeholder:text-muted-foreground focus:outline-none focus:border-accent transition-colors resize-none font-mono text-sm disabled:opacity-60"
                />
                    ...
                    >
                      ...
                      <div className="flex gap-2">
                        <input
                          ...
                          className="flex-1 px-4 py-2.5 bg-background border border-border rounded-xl text-foreground placeholder:text-muted-foreground focus:outline-none focus:border-accent transition-colors text-sm"
                        />
                        <button
                          type="button"
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                  <select
                    ...
                    className="w-full appearance-none px-4 py-3 pr-10 bg-background border border-border rounded-xl text-foreground focus:outline-none focus:border-accent transition-colors text-sm"
                  >
                ...
                {contentFilter === "bm25" && (
                  ...
                  >
                    ...
                    <input
                      ...
                      className="w-full px-4 py-3 bg-background border border-border rounded-xl text-foreground placeholder:text-muted-foreground focus:outline-none focus:border-accent transition-colors text-sm"
                    />
                  </motion.div>
                )}
              </AnimatePresence>
              ...
              >
                {isImporting ? (
                  ...
                  <>
                    ...
                    Import from Web{urls.length > 0 ? ` (${urls.length})` : ""}
                  </>
                )}
              </button>
            </>
