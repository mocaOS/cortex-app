...
- **A `.pptx`** with one full-bleed painted image per slide. Decks are built to stand on their own — each slide states its takeaway in its own title, so the pages carry the argument without a narrator.
...
- **A design system you approve from a finished page** — ~12 presets, and the sample you sign off on is a fully painted slide from *your* deck, not a color swatch. The page you approve then becomes the deck's **style anchor**: every remaining slide is generated against that image, so the whole deck reads as one visual system rather than twelve loosely related pictures. Approved styles save to a cross-project library.
- **Consistency from reference images** — optional subject uploads (1–3) keep a product, mascot or character recognizable across slides; the uploads bind its identity while each slide stages it in its own pose, action and camera angle. Style uploads lock the aesthetic.
...
3. **Style** — *the look gate.* Three presets are proposed with rationale, then one representative content slide is painted in full as a paid sample (one image; cached per preset, so comparing styles never re-pays). Approve, switch preset, or give feedback for a revised look. Approving it fixes the deck's look in the most literal way possible: that image rides along as the style reference for every slide that follows.
4. **Visuals** — *the cost gate.* Every missing slide image is quoted against live pricing and the run waits for confirmation. The approved sample already counts as generated. Each slide is painted against the style anchor, plus your subject reference on the slides that feature it.
5. **Assembly** — pure packaging into `.pptx` and PDF; nothing is generated here.
6. **QA** — structural probes (slide count matches the outline, images embedded, PDF pages), ending by naming what it cannot verify: the painted text. Proofread every slide.

Every artefact is cached on disk; a resume regenerates only what is missing.

**After delivery**, the deck stays editable at the right granularity: a text or artwork fix on one slide answers with a live one-image quote, then repaints exactly that slide — a full repaint from your corrected words, the style anchor and the subject reference, never a touch-up pass over the existing picture, which costs quality. That per-slide loop is the designed remedy for painted-text errors — not a reason to regenerate a deck.
