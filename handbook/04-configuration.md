...
| `OPENAI_MODEL` | `qwen3-6-35b-a3b` | Model name for Q&A, research, and chat. Recommended: Qwen3.6 35B A3B — a blazing-fast 35B/3B-active MoE, ideal for retrieval. MiniMax M3 can give slightly better results but costs the system its snappiness — not a worthwhile tradeoff. |
...
| `DEFAULT_REASONING_MODE` | `off` | Reasoning mode for the chat/answer path (speed researcher loop + writer + non-agentic/fast streaming). `off` suppresses hidden reasoning (Venice `disable_thinking`) for a sub-second first token and to avoid empty/timeout answers. On OpenAI GPT-5/o-series, `off` can disable parallel tool calls — set `auto` there. |
| `RESEARCH_REASONING_MODE` | `off` | Reasoning mode for the deep-research (quality) researcher loop. `off` because that loop reflects explicitly via its `reasoning` tool, which persists in the message history — hidden thinking is discarded between rounds yet competes for the same output budget. Set `auto` to restore provider-default thinking. |
...
OPENAI_MAX_CONTEXT=256000                  # Qwen3.6 35B A3B's full input window (= code default since 2026-07-09)
...
| `RESEARCHER_GIT_TOOL` | `auto` | When the agent gets the `git_repo` tool. `auto` — only when a repository is connected read/write, so the agent can actually propose changes; a read-only repo is already ingested, so questions about it are answered from the knowledge base (with citations) instead. `always` — whenever any repository is connected. `off` — never; ingestion is unaffected. |
