...
| `EXTRACTION_MAX_OUTPUT_TOKENS` | `12000` (set `0` to inherit `OPENAI_MAX_OUTPUT_TOKENS`, 8000) | Max output tokens for entity-extraction LLM calls. Sized to ≈ half of `GRAPH_EXTRACTION_MAX_CONTEXT`: entity-dense documents overflow an 8000 cap on 24K-token input batches (measured 2026-07-08); each overflow self-heals via split-retry but roughly doubles that batch's wall time. The backend logs a one-shot "output budget looks too small" warning when the ratio is off. Caveat: the cap must also decode inside the provider's request window (`LLM_REQUEST_TIMEOUT_SECONDS`) — on very slow gateways, lower `GRAPH_EXTRACTION_MAX_CONTEXT` instead of raising this further, or truncate-splits just become slower timeout-splits. |
...
OUTPUT TOKENS:                          INPUT CONTEXT:
  ...
  (EXTRACTION_MAX_OUTPUT_TOKENS ships     (extraction inherit clamped at 48K)
   a real default of 12000; 0 = inherit)
       ↓                                       ↓
...
EXTRACTION_MAX_OUTPUT_TOKENS=12000         # ≈ half the input budget (= code default since 2026-07-09) — entity-dense docs overflow 8000
# RELATIONSHIP_MAX_CONTEXT stays unset — inherits 24000. See its table entry before widening.
...
**Targeted overrides:**
- Adjust extraction tier output independently → `EXTRACTION_MAX_OUTPUT_TOKENS=12000` (recommended; keep it ≈ half of `GRAPH_EXTRACTION_MAX_CONTEXT` — undersizing causes overflow split-retries on entity-dense docs)
