...
**One-click start from Documents.** The Documents page's graph banner button sends you to `/extract?autostart=1`, and the Knowledge Graph page auto-runs the right flow on arrival (waits for its data fetch, then fires once). The URL is stripped to plain `/extract` immediately so a refresh won't re-fire. Which flow depends on graph state: with an existing graph the button is labeled **"Update Graph"** and runs the incremental Step 1 only (same as "Extract Entities" — pending documents are folded in, nothing is deleted, no confirm dialog; Steps 2/3 then show as needing an update). On a fresh instance it is labeled **"Generate Graph"** and runs the full 3-step chain. A full from-scratch rebuild is only ever triggered by the explicit "Regenerate Graph" button on the Knowledge Graph page.
