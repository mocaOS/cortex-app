...
3. **Mint a monetized public key** — in API key creation choose *Monetized public key*, set a price per query (e.g. `0.05` USDC) and a **deep research multiplier** (default `10×`; the modal previews both rates live). Quick asks and search bill flat; agentic deep-research queries bill at `price × multiplier`, quoted in the 402 challenge before the agent signs; `0` disables research on the key. Monetized keys are locked down at the API level: read-only by construction (never combinable with `manage`), restricted to `/api/search` and the Ask AI endpoints (no raw document/file access, no stats), still collection-scopable to sell exactly the slice of knowledge you choose.
