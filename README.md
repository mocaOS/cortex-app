...
- **💸 x402 Payments**: Monetize your knowledge base with pay-per-query **agentic micropayments** via the open [x402 standard](https://github.com/x402-foundation/x402). Free member keys keep working as before; in parallel, mint **monetized public keys** that agents pay per retrieval query in stablecoins (e.g. USDC on Base or Solana), with revenue flowing straight to a wallet you control — subsidize your members' inference and amortize your infra. Vendor-agnostic (any spec-compliant x402 facilitator), configured and **verified** entirely in the admin UI behind a single `X402_ENABLED=true` flag. Monetized keys are read-only, retrieval-endpoints-only, and collection-scopable — sell exactly the slice of knowledge you choose. See the [x402 Payments guide](documentation/pages/features/x402-payments.mdx).
