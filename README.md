...
| [**cortex-slides**](https://github.com/mocaOS/cortex-slides) | Turns a knowledge base into presentation decks (.pptx + PDF). Cortex research grounds an outline you approve word by word; gpt-image-2 (Venice.ai) then paints every slide in full — typography included — in a design system you confirm from one paid sample slide, which then anchors the look of the whole deck. Outline, style and cost gates keep a human in control of every dollar; single slides regenerate cheaply. Beta. |
