...
- Next.js App Router with unified navigation structure:
  ...
  - **Settings** (`/admin`): Statistics dashboard, system configuration (LLM Configuration section with 5 sub-areas: Primary Model, Extraction Model, Relationship Model, Vision Model, Embeddings — showing model names, API base URLs, context windows, and concurrency settings via `GET /api/admin/config`; no API keys exposed), API key management, danger zone (system reset). Stats bar hidden on this page.
